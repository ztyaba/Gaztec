...
export default function Footer() {
  return (
    <footer className="border-t border-white/10 bg-slate-950/60">
      <div className="mx-auto flex max-w-7xl flex-col gap-12 px-6 py-16 md:flex-row md:justify-between lg:px-8">
        <div className="max-w-md space-y-4">
          <div className="flex items-center gap-3">
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-500 to-indigo-500 text-lg font-bold text-white shadow-glow">
              GZ
            </span>
            <div>
          ...
          <div className="flex gap-4">
            {contactInfo.social.map((item) => (
              <Link key={item.name} href={item.href} className="text-sm text-slate-400 transition hover:text-white">
                {item.name}
              </Link>
            ))}
          </div>
        ...
        <div className="grid flex-1 grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-4 whitespace-pre-line text-sm text-slate-300">{contactInfo.headquarters.address}</p>
            <p className="mt-2 text-sm text-slate-300">{contactInfo.headquarters.phone}</p>
            <Link href={`mailto:${contactInfo.headquarters.email}`} className="mt-2 block text-sm text-white">
      ...
      <div className="border-t border-white/5 bg-slate-950/80 py-6">
        <p className="mx-auto max-w-7xl px-6 text-xs uppercase tracking-[0.3em] text-slate-500 lg:px-8">© {new Date().getFullYear()} Gaztec Forge. Engineered with precision.</p>
      </div>
    </footer>
  );
}
